...
(function() {
	...
	**/
	...
	// constructs function to get/put/delete entries from hash
		...
		**/
		...
		var submit_query = function(s_query, f_okay) {
			jQuery.ajax({
				...
				success: f_okay? function(z_res) {
					if(typeof z_res === 'object') {
						f_okay(z_res.results.bindings);
					}
					else {
						f_okay(z_res);
					}
		...
		var qb = function(h_query) {
			...
			var f_exec = function(f_okay) {
				var s_query = f_to_sparql();
				if(false) s_query = s_query.replace(/[\n\t]+/g, ' ');
			...
			var d_self = {
				...
				values: function(z_map) {
					if('object' === typeof z_map) {
						for(var s_var in z_map) {
							var z_items = z_map[s_var];
							if(Array.isArray(z_items)) {
								var a_values = z_items.map(function(s_item) {
									return rdf_lit(s_item);
								});
								h_query.tail += '\nVALUES '+rdf_var(s_var)+' { '+a_values.join(' ')+' }';
							}
						}
						return d_self;
					}
				...
				results: function(f_okay) {
					f_exec(function(h_res, h_status, v_xhr) {
						// console.info(h_res);
						// if(f_okay) {
						// 	if((v_xhr.getResponseHeader('content-type') || '').indexOf('json')) {
						// 		f_okay()
						// 	}
						// }
						f_okay && f_okay(h_res);
		...
		**/
		var operator = function(z_query) {
			...
			// string
				...
				if(z_query[0] === ':' || z_query[0] === '?' || z_query[0] == '<') {
					return qb({
						select: '',
						where: '\n'+z_query,
						tail: '',
				...
				else if(m_query=/^\s*describe\s(.+)$/i.exec(z_query)) {
					return query_string(sq_prefixes+' '+z_query);
				}
		...
		**/
			...
			operator['prefix'] = hash_gpd(h_prefixes, function() {
				...
				for(var e in h_prefixes) {
					...
					h_urls[h_prefixes[e]] = e;
				}
			});

			// remote controlled prefixes
			operator['prefixes'] = function(s_format) {
				s_format = s_format.toLowerCase();
				if(arguments.length === 0 || s_format === 'sparql') {
					return sq_prefixes;
				}
				else if(s_format === 'turtle' || s_format === 'ttl') {
					var s_ttl = '';
					for(var e in h_prefixes) {
						s_ttl += '@prefix '+e+': <'+h_prefixes[e]+'> .\n';
					}
					return s_ttl;
				}
			};
			...
			// expand the given turtle identifier
			operator['expand'] = function(s_ttl_uri) {
				if(m_ttl=/^([^:]*):([a-zA-Z_]\w*)$/.exec(s_ttl_uri)) {
					if(h_prefixes.hasOwnProperty(m_ttl[1])) {
						return h_prefixes[m_ttl[1]]+m_ttl[2];
					}
				}
				else if(/^[a-zA-Z_]\w*$/.test(s_ttl_uri) && h_prefixes.hasOwnProperty('')) {
					if(s_ttl_uri === 'a') return operator.expand('rdf:type');
					return h_prefixes['']+s_ttl_uri;
				}
				return s_ttl_uri;
			};

			// shorten the given uri
			operator['shorten'] = function(s_uri) {
				if(m_key=/^(http.*)\b(\w+)/.exec(s_uri)) {
					if(h_urls.hasOwnProperty(m_key[1])) {
						return h_urls[m_key[1]]+':'+m_key[2];
					}
				}
				return s_uri;
			};
	...
	**/
	...
	(function() {
		...
		exports['prefixes'] = global_instance('prefixes');
		exports['endpoint'] = global_instance('endpoint');
		exports['load'] = global_instance('load');
		exports['expand'] = global_instance('expand');
		exports['shorten'] = global_instance('shorten');
	})();
